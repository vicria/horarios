...
public abstract class TextMessage {
    ...
    public EventDto workTime = new EventDto(new DateTime(2023, 7, 7, startTime, 00),
            new DateTime(2023, 7, 7, endTime, 00));

    public List<Week> workDays = Arrays.asList(Week.LUNES, Week.MARTES, Week.MIERCOLES, Week.JUEVES);
    ...
    List<EventDto> inverse(List<EventDto> events) {
        if (!events.isEmpty()) {
            ...
            DateTime dateTime = new DateTime(start.getYear(), start.getMonthOfYear(), start.getDayOfMonth(), startTime, 00);
            workTime.setStart(dateTime);

            DateTime end = events.get(0).getEnd();
            DateTime dateTime2 = new DateTime(end.getYear(), end.getMonthOfYear(), end.getDayOfMonth(), endTime, 00);
